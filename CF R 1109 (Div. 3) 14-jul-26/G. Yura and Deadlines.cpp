...
public:
    ...
    Node convert(int x){ return Node(x); }
    Node combine(const Node& a, const Node& b){ return Node(max(a.value, b.value)); }
...
int solve() {
    ...
    priority_queue<pii, vector<pii>, greater<pii>> operaciones;
    vi dp(n+1);

    vi data(n); rep(i, n) data[i] = 0;
    SegmentTree arbol(data);

    for (int i = 0; i<n; i++){
        while(operaciones.size() && operaciones.top().first < i){
            arbol.update(operaciones.top().second, dp[operaciones.top().second]);
            operaciones.pop();
        }
        if (i-a[i] >= 0) dp[i] = a[i] + arbol.query(0, i-a[i]-1).value;
        else dp[i] = a[i];
        operaciones.push(pii(i + a[i], i));
    }
    int sol = 0;
